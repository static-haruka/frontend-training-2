...
type Props = {
  stack: VpcStack;
  onClose: () => void;
  onUpdate: (id: string, data: Partial<VpcStack>) => void;
};
...
export default function EditModal({ stack, onClose, onUpdate }: Props) {
  const [stackName, setStackName] = useState(stack.stackName);
  const [status, setStatus] = useState(stack.status);
  const [description, setDescription] = useState(stack.description);

  const handleSubmit = () => {
    if (!stackName || !status) return;
    onUpdate(stack.id, { stackName, status, description });
    onClose();
  };
